...
type ApiRole = "SUPER_ADMIN" | "TOURNAMENT_ADMIN" | "SCORER" | "VIEWER";
...
const roleDisplayMap: Record<ApiRole, { label: string; className: string }> = {
  SUPER_ADMIN: { label: "Admin", className: "bg-danger/10 text-danger" },
  TOURNAMENT_ADMIN: { label: "Tournament Admin", className: "bg-warning/10 text-warning" },
  SCORER: { label: "Scorer", className: "bg-accent/10 text-accent" },
  VIEWER: { label: "Viewer", className: "bg-muted/10 text-muted" },
};

const filterToApiRole: Record<string, ApiRole | null> = {
  Admin: "SUPER_ADMIN",
  Scorer: "SCORER",
  Viewer: "VIEWER",
  All: null,
  ...
  useEffect(() => {
    async function fetchUsers() {
      ...
      try {
        ...
        const apiRole = filterToApiRole[activeRole];
              ...
              {loading
                ? Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i}>
                      ...
                      <td className="px-5 py-3"><SkeletonBlock className="h-4 w-20 ml-auto" /></td>
                    </tr>
                  ))
                : users.map((user) => (
                    <tr key={user.id} className="transition-colors hover:bg-white/[0.02]">
                      ...
                      <td className="px-5 py-3">
                        <span className={cn("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold", roleDisplayMap[user.role]?.className ?? roleDisplayMap.VIEWER.className)}>
